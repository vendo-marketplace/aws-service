...
import com.vendo.aws_service.adapter.security.in.filter.path.InternalAntPathResolver;
...
public class InternalFilter extends OncePerRequestFilter {
    ...
    private TokenClaims validateClaims(String token) {
        ...
        if (CollectionUtils.isEmpty(claims.audience()) || !claims.audience().contains(ServiceName.AWS_SERVICE.getServiceName())) {
